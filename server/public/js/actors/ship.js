...
Actors.Ship.prototype.runSystem = function(){
  ...
  for(var i=0, ii=this.refs.system.ships.length; i<ii; i++){
    if(this.refs.system.ships[i] === this){
      continue;
    }
    if(this.refs.system.ships[i].refs.species !== this.refs.species){
      enemies.push(this.refs.system.ships[i]);
      enemy_ships ++;
    } else {
      friends.push(this.refs.system.ships[i])
      friend_ships ++;
    }
  }
...
Actors.Ship.prototype.star_gravity = function(){
  ...
  this.refs.system.stars.forEach(function(star){
    r = this.pos.range(star.pos);
    g = star.pos.minus(this.pos).normalize().scale(this.opts.star_gravity).scale(1/r); ;
    vec.add(g);
...
Actors.Ship.prototype.planet_gravity = function(){
  ...
  this.refs.system.planets.forEach(function(planet){
    r = this.pos.range(planet.pos);
    g = planet.pos.minus(this.pos).normalize().scale(this.opts.planet_gravity).scale(1/r); ;
    vec.add(g);
