...
Scenes.trap.prototype.update = function(delta){
  ...
  if(this.attrs.hold > 0){
    ...
    if(this.attrs.hold <= 0){
      ...
      if(this.attrs.frame_index === Scenes.trap.prototype.frames.length){
    ...
    if (this.attrs.time > this.opts.step_hold) {
      ...
      if (this.attrs.step_index >= Scenes.trap.prototype.frames[this.attrs.frame_index].text.length) {
...
Scenes.trap.prototype.paint = function(fx, gx, sx){
  ...
  var frame = Scenes.trap.prototype.frames[this.attrs.frame_index];
...
Scenes.trap.prototype.frames = [];

Scenes.trap.prototype.frames[0] = {
  text:[
    'King Rat',
  ].join("\n"),
};
