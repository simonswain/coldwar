/*global Scenes:true, Actors:true, pickOne:true */
/*jshint browser:true */
/*jshint strict:false */
...
Scenes.interception.prototype.update = function(delta){
  ...
  while(this.bombs.length < this.opts.bombs_max && bombs_this_tick < this.opts.bombs_per_tick){
    bombs_this_tick ++;
    this.bombs.push(new Actors.Bomb(this.env, {
      world: this,
      color: '7',
...
Scenes.interception.prototype.paint = function(fx, gx){
  ...
  for(var i = 0, ii=this.silos.length; i<ii; i++){
    gx.ctx.save();
  ...
  for(var i = 0, ii=this.bombs.length; i<ii; i++){
    gx.ctx.save();
  ...
  for(var i = 0, ii=this.interceptors.length; i<ii; i++){
    gx.ctx.save();
