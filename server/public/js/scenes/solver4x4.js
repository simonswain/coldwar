...
Scenes.solver4x4.prototype.solve = function(delta){
  ...
  if(length = open.length) {
    ...
    min = -1;
    for(i = 0; i < length; i++) {
      if(nodes[open[i]].f < max) {
        max = nodes[open[i]].g + nodes[open[i]].h;
        min = i;
      }
    }
...
Scenes.solver4x4.prototype.paintSolve = function(fx, gx, sx){
  ...
  var x, y;
  var max_f = 0;
  for(var i = 0, ii=this._solve.nodes.length; i < ii; i++){
    if(this._solve.nodes[i].f > max_f){
      max_f = this._solve.nodes[i].f;
    }
  }
  ...
  for(var i = 0, ii=this.attrs.rows * this.attrs.cols; i < ii; i++){
    ...
    if(this._solve.nodes[i].g > 0){
      ...
      gx.ctx.fillStyle = 'rgba(0,0,255,' + ( this._solve.nodes[i].f / max_f) + ')';
      gx.ctx.beginPath();
      gx.ctx.fillRect((x * ww), (y * hh), ww, hh);
    }
    ...
    if(i !== 0 && this._solve.nodes[i].f > 0){
      gx.ctx.fillStyle = '#c00';
      gx.ctx.font = '18pt robotron';
      gx.ctx.textAlign = 'center';
      gx.ctx.textBaseline = 'middle';
      gx.ctx.fillStyle = '#fff';
      gx.ctx.fillText(this._solve.nodes[i].g + this._solve.nodes[i].h, (x * ww) + ww/2, (y * hh) + hh*0.5);
      // gx.ctx.font = '12pt robotron';
      // gx.ctx.fillStyle = '#000';
      // gx.ctx.fillText(this._solve.nodes[i].g, (x * ww) + ww * 0.25, (y * hh) + hh/4);
      // gx.ctx.fillText(this._solve.nodes[i].h, (x * ww) + ww * 0.75, (y * hh) + hh/4);
    };
