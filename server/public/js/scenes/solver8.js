...
Scenes.solver8 = function(env, opts){
  this.env = env;
...
Scenes.solver8.prototype = Object.create(Scene.prototype);

Scenes.solver8.prototype.title = 'Solver8';

Scenes.solver8.prototype.genAttrs = function(){
  return {
    rows: this.opts.rows,
    cols: this.opts.cols,
    phase: 'gen'
...
Scenes.solver8.prototype.init = function(){
...
Scenes.solver8.prototype.defaults = [{
  key: 'max_x',
  value: 480,
  min: 32,
  max: 1024
...
Scenes.solver8.prototype.makeGrid = function () {
  // init blank grid
...
Scenes.solver8.prototype.generatePerfectMaze = function () {
  var stack = [];
...
Scenes.solver8.prototype.update = function(delta){
}

Scenes.solver8.prototype.paint = function(fx, gx, sx){
  ...
  for(var i = 0, ii=this.attrs.rows * this.attrs.cols; i < ii; i++){
    ...
    if(i === 11){
      ...
      gx.ctx.lineWidth=4;
      gx.ctx.beginPath(); 
      gx.ctx.moveTo(3.5 * ww, 3 * hh);
      gx.ctx.lineTo(3.5 * ww, 0.5 * hh);
      gx.ctx.lineTo(1 * ww, 0.5 * hh);
      gx.ctx.stroke();

      gx.ctx.beginPath(); 
      gx.ctx.moveTo(1 * ww, 0.5 * hh);
      gx.ctx.lineTo(1.25 * ww, 0.25 * hh);
      gx.ctx.stroke();

      gx.ctx.beginPath(); 
      gx.ctx.moveTo(1 * ww, 0.5 * hh);
      gx.ctx.lineTo(1.25 * ww, 0.75 * hh);
      gx.ctx.stroke();
